package com.flowdesk.auth.application.service.impl;
...
public class AuthServiceImpl implements AuthService {
     ...
     */
    @Override
    public IssuedSessionResult login(LoginCommand loginCommand) {
        // 认证
    ...
    AuthenticationSnapshot authenticate(LoginCommand loginCommand) {
        AuthenticationSnapshot authentication = iamAuthService.findByUsername(loginCommand.username());

        if (authentication == null
                || authentication.status() != IamUserStatus.ENABLED
                || !passwordEncoder.matches(loginCommand.password(), authentication.password())) {
            log.warn("SECURITY login-failed username={} result=AUTH_INVALID_CREDENTIALS", loginCommand.username());
     ...
     */
    @Override
    public IssuedSessionResult refresh(String refreshToken) {
    ...
    @Override
    public AuthenticatedUserResult currentUser(AuthPrincipal principal) {
        AuthSession session = requireActiveSession(principal.sessionId());
        return currentUserView(session);
    }
     ...
     */
    @Override
    public void changePassword(AuthPrincipal principal, ChangePasswordCommand command) {
        // 1. 与登录同一口径校验原密码：用户不存在、账号停用、密码错误都算失败
        AuthenticationSnapshot authentication = iamAuthService.findByUsername(principal.username());
        if (authentication == null
                || authentication.status() != IamUserStatus.ENABLED
                || !passwordEncoder.matches(command.currentPassword(), authentication.password())) {
        ...
        if (!iamUserService.updatePassword(principal.userId(), passwordEncoder.encode(command.newPassword())))
            throw new ApiException(HttpStatus.CONFLICT, "USER_CONFLICT", "用户信息已变化，请重新登录后再试");

    }
    /** 签发 Access Token 并组装响应：登录与刷新共用同一响应形状。 */
    private IssuedSessionResult issue(AuthSession session, String refreshToken) {
        // 签发 Access Token
        AccessToken accessToken = jwtTokenService.issue(session.userId(), session.sessionId());
        // token传递到前端Js
        AuthenticatedUserResult user = currentUserView(session);
        // Refresh 只进cookie
        LoginResult response = new LoginResult(accessToken.value(), LoginResult.BEARER,
                jwtProperties.expiration().toSeconds(), user);
        // 组装响应
        return new IssuedSessionResult(response, refreshToken);
    }
    ...
    /**
     * 当前用户视图：用户可能停用、删除，此时应撤销所有会话并返回 401。
     */
    private AuthenticatedUserResult currentUserView(AuthSession session) {
        UserProfileSnapshot profile =
                iamAuthService.findProfileById(session.userId());

        if (profile == null
                || profile.status() != IamUserStatus.ENABLED) {
            authSessionRepository.revokeAll(session.userId());
            throw sessionInvalid();
        }
