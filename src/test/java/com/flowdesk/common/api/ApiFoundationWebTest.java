...
import com.flowdesk.common.web.filter.TraceIdFilter;
...
@Import({ApiFoundationWebTest.ProbeConfiguration.class, MockedPersistenceConfiguration.class})
class ApiFoundationWebTest {
    ...
    @Test
    @WithMockUser
    void unreadableRequestBodyUses400Envelope() throws Exception {
        mockMvc.perform(post("/fd/v1/test-probe/validation")
                        .header(TraceIdFilter.HEADER_NAME, TRACE_ID)
                        .contentType("application/json")
                        .content("{\"name\":"))
